	...
	<Carousel.Content class="-ml-4 py-4">
		{#each movies as movie (movie.id)}
			<Carousel.Item class="pl-4 basis-[55%] md:basis-[30%] lg:basis-[22%] xl:basis-[18%]">
				<button
					class="group flex w-full cursor-pointer flex-col text-center focus:outline-none transition-transform duration-300 hover:scale-[1.03] hover:z-10"
					onclick={() => openMovieDetails(movie)}
				>
					<div class="relative w-full overflow-hidden rounded-2xl ring-1 ring-white/10 shadow-lg transition-all duration-300 group-hover:ring-white/30 group-hover:shadow-[0_8px_30px_rgb(255,255,255,0.12)]">
						{#if movie.label}
						...
						<div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4 pt-12 text-left">
